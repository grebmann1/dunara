...
const storeSchema = z.object({ version: z.literal(1), preferences: z.array(preferenceSchema).max(100), rows: z.array(rowSchema).max(100) }).strict();
...
export class AssistantDrafts {
  ...
  private load() { return this.store.load() ?? { version: 1 as const, preferences: [], rows: [] }; }
  ...
  configure(scope: DraftScope, input: unknown) {
    ...
    if (!value.enabled) data.rows = data.rows.filter(row => row.owner !== owner);
    this.store.save(data); return this.read(scope);
  }
