...
export const CHATGPT_IMAGE_MODEL = 'chatgpt-image';
export const mediaModelSchema = z.enum([IMAGE_MODEL, ASTRA_MODEL, CHATGPT_IMAGE_MODEL]);
export const jobRequestSchema = z.object({
  ...
  .refine(v => v.model === IMAGE_MODEL || v.count === 1, 'This connection supports one candidate per approved request');
...
export const mediaModelLabel = (model: z.infer<typeof mediaModelSchema>) => model === CHATGPT_IMAGE_MODEL ? 'ChatGPT · built-in image generation' : model === ASTRA_MODEL ? `${ASTRA_MODEL} → ${IMAGE_MODEL}` : model;
