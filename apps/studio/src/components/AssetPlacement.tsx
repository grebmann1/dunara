...
export type AssetPlacementDraft = AssetPlacement & { expanded: boolean; requested?: boolean };
export function AssetPlacementPreview({ projectId, asset, state, value, onChange }: { projectId: string; asset: Asset; state?: StudioState; value: AssetPlacementDraft; onChange(value: AssetPlacementDraft): void }) {
  ...
  return <details className="asset-placement" open={value.expanded} onToggle={event => { if (event.currentTarget.open !== value.expanded) onChange({ ...value, expanded: event.currentTarget.open }); }}><summary>Preview placement & crop</summary>
    ...
    <FieldSelect label="Target screen" value={value.route} onValueChange={route => onChange({ ...value, route, requested: true })} options={(state?.screens?.length ? state.screens : [{ name: 'Home', route: '/' }]).map(screen => ({ value: screen.route, label: screen.name }))} />
    <FieldSelect label="Placement" value={value.role} onValueChange={role => onChange({ ...value, role: role as AssetPlacement['role'], requested: true })} options={[{ value: 'hero', label: 'Hero artwork' }, { value: 'avatar', label: 'Character / avatar' }, { value: 'background', label: 'Screen background' }]} />
    <FieldSelect label="Image fit" value={value.fit} onValueChange={fit => onChange({ ...value, fit: fit as AssetPlacement['fit'], requested: true })} options={[{ value: 'contain', label: 'Keep the entire image' }, { value: 'cover', label: 'Fill and crop' }]} />
