...
type Props = { renderTools: (screenActions: ReactNode) => ReactNode; phoneControls?: ReactNode; onAskAssistant?: (attachment: InspectorAttachment) => void; onAskScreens?: (screens: Screen[]) => void; onRevealContext?: () => void; suppressContext?: boolean; preview: Preview; project: Project; board: PreviewBoardState; screens: Screen[]; captures: BoardCapture[]; sourceRevision: string; capturePaused?: boolean; candidates: string[]; onChange: (action: BoardAction) => Promise<boolean>; active: boolean; disabled: boolean; perform: (label: string, operation: () => Promise<unknown>) => Promise<boolean> };
export function PreviewBoard({ renderTools, phoneControls, preview, project, board, screens: discovered, captures, sourceRevision, capturePaused = false, candidates, onChange, active, disabled, perform, suppressContext = false, onRevealContext, onAskAssistant, onAskScreens }: Props) {
  ...
  return <div className="device-stage preview-workspace" data-mode={board.mode}>
    ...
    <ScreenOverview controlsHost={overviewControls} projectId={project.id} screens={overviewScreens} captures={captures} sourceRevision={sourceRevision} active={active && board.mode === 'overview'} ready={preview.status === 'ready'} disabled={disabled || capturePaused} onFocus={route => onChange({ type: 'focus-screen', route })} onCompare={routes => comparison('screens', routes[0], routes[1])} onAsk={onAskScreens} />
