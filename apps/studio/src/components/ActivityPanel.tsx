...
export function ActivityPanel({ projectId, data, state, onReview, onResult, onLaunchKit, onAssets, onBackend }: { projectId: string; data: MediaState; state?: StudioState; onReview(id: string): void; onResult(id: string): void; onLaunchKit(): void; onAssets(): void; onBackend(): void }) {
  ...
  return <>
    ...
    <div className="media-workspace-toolbar" role="group" aria-label="Activity categories">{[['backend', 'Backend'], ['requests', `Requests · ${jobs.length}`], ['diagnostics', `Diagnostics${errors ? ` · ${errors}` : ''}`], ['captures', `Captures · ${state?.captures.length ?? 0}`]].map(([value, title]) => <Button key={value} variant="ghost" aria-pressed={tab === value} onClick={() => setTab(value!)}>{title}</Button>)}{(pending > 0 || failures > 0) && <Button className="activity-attention" variant="outline" onClick={() => { setTab('requests'); setFilter(pending ? 'active' : 'failed'); setQuery(''); }}>{pending ? `${pending} active` : `${failures} need attention`}</Button>}</div>
