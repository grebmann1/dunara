...
export function SettingsPanel({ initialSection, enabledPlugins, settings, onSettings, disabled, project }: { initialSection?: 'images'; enabledPlugins?: string[]; project?: Project; settings?: ProviderStatus; onSettings: (value: ProviderStatus) => void; disabled: boolean }) {
  ...
  return <main className="destination settings-workspace">
    ...
    <section ref={imageSection} tabIndex={-1} className="settings-section" aria-label="OpenAI configuration">
      ...
      <p>ChatGPT sign-in connects Assistant and Suggest. To create images in Dunara, {settings?.managed ? `choose ${settings.managed.label} or use an OpenAI API key.` : 'add an OpenAI API key.'} API usage is billed separately from your ChatGPT subscription. <a href="https://help.openai.com/en/articles/9039756" target="_blank" rel="noreferrer">About ChatGPT and API billing</a></p><p>Image connection: <strong>{settings?.source ?? 'Loading…'}</strong>. Choose the Assistant provider and model in AI connections.</p>
