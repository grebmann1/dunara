...
export function LaunchKitPanel({ projectId, state, media, active, savedDraft, onDraft }: { projectId: string; state?: StudioState; media: MediaState; active: boolean; savedDraft?: LaunchKitDraft; onDraft: (draft: LaunchKitDraft) => void }) {
  ...
  return <section hidden={!active} className="launch-kit-panel" aria-label="Launch Kit">
    ...
    <p className="muted">React Native Web — not native App Store screenshots. Fresh-context captures do not reproduce the visible phone’s input or storage state. Review their revision evidence before sharing.</p>
    <details className="media-card"><summary>What remains before release?</summary><h3>iPhone / App Store</h3><p>Check a signed release on an iPhone through TestFlight. Prepare native screenshots, privacy disclosures, age rating, review access and each listing language in App Store Connect.</p><h3>Android / Google Play</h3><p>Prepare a store-signed AAB and complete internal testing, data safety, content rating, native screenshots and localized listings in Play Console. The preview APK below the build setup is for device testing.</p><h3>Web preview</h3><p>Check routes, primary actions, persistence, access control and the chosen backend environment. A local or private preview URL is not a public deployment.</p><p>The exported readiness checklist tracks these destinations separately. Dunara does not submit this kit to a store.</p></details>
