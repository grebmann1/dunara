...
import { ASTRA_MODEL, CHATGPT_IMAGE_MODEL, mediaModelLabel } from '../../../../packages/core/src/media-job-contracts';
...
export function JobCard({ job, data, projectId, focused, disabled, save, onResult, onRemix }: { job: MediaState['jobs'][number]; data: MediaState; projectId: string; focused: boolean; disabled: boolean; save: (action: string, input: unknown) => Promise<boolean>; onResult: (id: string) => void; onRemix?: (asset: Asset) => void }) {
  ...
  const chatgpt = job.model === CHATGPT_IMAGE_MODEL;
  const waiting = job.state === 'awaiting-approval';
  const modelAvailable = data.capabilities.models.some(model => model.id === job.model);
  const disclosure = <>
    {waiting && !modelAvailable && <p role="alert">This request’s model is unavailable with the selected connection. Create a request with an allowed model or explicitly select a personal image key in Settings. No provider call has been made.</p>}
    {waiting && modelAvailable && data.capabilities.provider.source === 'managed' && <p>{job.creditEstimate === undefined ? 'Credit estimate unavailable. Choose a supported request or a personal image key.' : `Estimated reservation: ${job.creditEstimate.toLocaleString(undefined,{ maximumFractionDigits: 2 })} credits. Actual reported usage is charged; unused credit is released.`}</p>}
  ...
  return <article ref={card} tabIndex={-1} aria-label={`${job.request.label} request`} className="media-card media-job" data-state={job.state}><div className="creative-job-heading"><span className="creative-job-mark" aria-hidden>{job.state === 'succeeded' ? <Check size={20} /> : job.state === 'running' ? <LoaderCircle size={20} className="assistant-spinner" /> : <Sparkles size={20} />}</span><div><h3>{job.request.label} · {waiting && !modelAvailable ? 'Choose image connection' : stateLabels[job.state]}</h3><p>{job.request.size.replace('x', ' × ')} · {job.request.quality} quality · {job.request.count} {job.request.count === 1 ? 'image' : 'images'}</p></div></div>{job.error && <p role="alert">{job.error}</p>}
    ...
    <fieldset disabled={disabled}><div className="media-actions">{waiting && <><Label className="consent"><input type="checkbox" disabled={!modelAvailable} checked={consent} onChange={e => setConsentRevision(e.target.checked ? revision : '')} />{chatgpt ? 'Use my ChatGPT allowance for this image request.' : `I authorize this exact paid request and disclosure using the current ${data.capabilities.provider.source} configuration.`}</Label><Button disabled={!consent || !data.capabilities.available || !modelAvailable} onClick={() => void save('job-approve', { jobId: job.id, expectedConfigurationRevision: consentRevision })}>{chatgpt ? 'Generate with ChatGPT' : 'Approve paid request'}</Button></>}{['awaiting-approval', 'queued', 'running'].includes(job.state) && <Button variant="outline" onClick={() => void save('job-cancel', { jobId: job.id })}>Cancel local job</Button>}</div></fieldset>
