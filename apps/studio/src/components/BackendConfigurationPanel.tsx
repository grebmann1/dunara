...
export function BackendConfigurationPanel({ projectId, environment, disabled, rememberAvailable, refresh }: { projectId: string; environment: EnvironmentName; disabled: boolean; rememberAvailable: boolean; refresh: () => Promise<void> }) {
  ...
  return <><FunctionEnvironmentPanel key={`${projectId}:${environment}`} projectId={projectId} environment={environment} disabled={disabled} rememberAvailable={rememberAvailable} refresh={refresh} /><section className="backend-card" aria-label="App services configuration"><h2>App services</h2><p>Configure email sign-in, delivery, private uploads, and functions from your app’s backend/configuration.json.</p>
    ...
    <div className="backend-actions"><Button variant="outline" disabled={disabled || busy} onClick={() => void run(inputs)}>Check required inputs</Button><Button disabled={disabled || busy} onClick={() => void run(async () => { const plan = await api<ConfigurationPlan>(`${base}/plan`, { action: 'configure', environment }); await api(`${base}/apply`, { plan, requestId: crypto.randomUUID() }); })}>Review service changes</Button><Button variant="outline" disabled={disabled || busy} onClick={() => void run(async () => { const value = await api<Report>(`${base}/validate`, { environment }); if (alive.current) setReport(value); })}>Validate configuration</Button></div>
