...
export function FunctionEnvironmentPanel({ projectId, environment, disabled, rememberAvailable, refresh, onReview }: { projectId: string; environment: EnvironmentName; disabled: boolean; rememberAvailable: boolean; refresh(): Promise<void>; onReview?: () => void }) {
  ...
  return <section className="backend-card backend-function-environment" aria-label="Supabase environment variables"><h2>Environment variables</h2><p>Private values for your Supabase Edge Functions · <strong>{environment}</strong></p>
    <ol><li>Add a variable name.</li><li>Save its value privately.</li><li>Prepare the change, then approve it in Reviews.</li></ol>
    <p className="backend-muted">All Edge Functions in this Supabase project share these values.</p>
