...
export function DeviceConnection({ preview: incoming, appName, onRefresh }: { preview: Preview; appName: string; onRefresh(): void }) {
  ...
  return <section className="device-preview" aria-label="Phone preview">
    ...
    <section className="device-preview-account" aria-label="Expo sign-in setup"><h3>First, sign in to Expo</h3><p>On iPhone, sign in to the same Expo account on your computer and in Expo Go.</p><p className="device-preview-note">Run this once in your app’s project folder:</p><div className="device-login-command"><code>{expoLoginCommand}</code><Button variant="outline" aria-label="Copy Expo login command" onClick={() => { void navigator.clipboard.writeText(expoLoginCommand).then(() => setNotice('Expo login command copied. Run it in your app’s project folder.')).catch(() => setNotice('Copy the command shown above into your terminal.')); }}><Copy size={16} aria-hidden />Copy</Button></div><p className="device-preview-note">Already signed in on both devices? {url ? 'Scan below.' : 'Start your phone preview below.'} Expo sign-in is separate from ChatGPT and your app’s account.</p></section>
    {url ? <>
    ...
    <details className="device-expo-login"><summary>Expo asks you to log in on your computer?</summary><p>Open a terminal in this app’s project folder and run <code>npx expo login --browser</code>. Complete Expo’s browser sign-in, using the same account as Expo Go on your phone, then scan again. If you change accounts, stop and start the preview first.</p><p>Your Expo account is separate from ChatGPT and your app’s own sign-in.</p></details>
