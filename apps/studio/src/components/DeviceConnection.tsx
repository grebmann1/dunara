...
export function DeviceConnection({ preview: incoming, appName, onRefresh }: { preview: Preview; appName: string; onRefresh(): void }) {
  ...
  return <section className="device-preview" aria-label="Phone preview">
    ...
    <ol className="device-preview-steps"><li>Install <a href="https://expo.dev/go" target="_blank" rel="noreferrer">Expo Go</a> on your phone. Connect both devices to the same Wi-Fi.</li><li>On iPhone, sign in to the same Expo account on your computer and in Expo Go.</li><li>{url ? <>Scan the QR code above.</> : <>Choose <strong>{startLabel}</strong>, then scan the QR code.</>} Keep Dunara running.</li><li>Edit the app in Dunara. Expo Fast Refresh sends saved code changes to your phone. If a change is missing, reload in Expo Go.</li></ol>
