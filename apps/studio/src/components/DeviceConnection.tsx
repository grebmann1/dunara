...
export function DeviceConnection({ preview: incoming, appName, onRefresh }: { preview: Preview; appName: string; onRefresh(): void }) {
  ...
  return <section className="device-preview" aria-label="Phone preview">
    ...
    </> : <div className="device-preview-empty"><h3>Test on your phone</h3><p>{preview.deviceIssue ?? (preview.status === 'starting' ? 'Your preview is starting. Its address will appear when it is ready.' : 'Start a phone preview to get a QR code for this app.')}</p></div>}
    <ol className="device-preview-steps"><li>Install <a href="https://expo.dev/go" target="_blank" rel="noreferrer">Expo Go</a> on your phone. Connect both devices to the same Wi-Fi.</li><li>{url ? <>Scan the QR code above.</> : <>Choose <strong>Start phone preview</strong>, then scan the QR code.</>} Keep Dunara running.</li><li>Edit the app in Dunara. Expo Fast Refresh sends saved code changes to your phone. If a change is missing, reload in Expo Go.</li></ol>
    ...
    <Button disabled={busy || preview.status === 'starting'} variant={url ? 'outline' : 'default'} onClick={() => void run('transport', { transport: url ? 'localhost' : 'lan', expectedSessionId: preview.sessionId ?? null })}>{busy ? 'Updating preview…' : url ? 'Use this computer only' : 'Start phone preview'}</Button>
    {error && <p role="alert">{error}</p>}
    <dl className="device-preview-details"><div><dt>Backend</dt><dd>{preview.environment ?? 'Not connected'}</dd></div>{preview.backendUrl && <div><dt>Project</dt><dd>{new URL(preview.backendUrl).hostname.split('.')[0]}</dd></div>}<div><dt>Connection</dt><dd>{url ? 'Local network' : 'This computer'}</dd></div>{preview.sessionId && <div><dt>Preview session</dt><dd>{preview.sessionId.slice(0, 8)}</dd></div>}</dl>
