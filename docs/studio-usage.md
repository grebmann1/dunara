...
The authenticated **Studio** controls real registered Expo projects. The separate marketing website and its animated phone are illustrations: they do not connect to projects or run code. Studio has seven destinations: **Preview** for running, inspecting, designing and capturing; **Assets** for images, art direction and paid-request review; **App Icons** for preparation and config review; **Backend** for Supabase setup; **Activity** for project diagnostics and media jobs; **Plugins** for installed capabilities; and global **Settings** for AI connections, image generation and other services. Settings works without a project. Your coding-agent harness inspects/edits source through MCP. Desktop Studio also has an optional side-panel Assistant using the same MCP capabilities; standalone Studio does not enable it. There is no source editor. See `docs/assistant.md`.
...
2. Select a registered project or choose **+ New app**. Enter a name and an optional brief, then choose **Create app**. Backend setup is optional: **App settings → Accounts & shared data** opens the Supabase connection flow. The default starts without a backend. Creating a Supabase-backed workspace opens Backend for selecting and reviewing its project connection; the wizard does not provision or approve remote changes. Names accept letters, numbers, spaces, periods, apostrophes and hyphens. Slugs are lowercase letters/numbers separated by single hyphens. Creation uses the wellness starter as editable source. When Assistant is available, Studio opens it and carries a non-empty brief into a project-scoped Build draft. Review and send the request to generate the first version; creation alone makes no model request. Other projects’ unsent drafts are retained. Cancel does not create a project. Duplicate slugs produce a retryable error.
...
A missing, unreadable or invalid project folder appears under **Unavailable apps · Recovery** while healthy projects remain usable. If the selected app disappears, Studio selects a healthy app or returns to creation when none remain. Restore the original folder at the displayed location and choose **Check again**. Dunara keeps the registration until you explicitly confirm **Remove from list**; that action only changes the registry and never deletes source. If a folder becomes available before removal, the server keeps its registration. Re-registering the restored folder preserves its project ID. Relinking to an arbitrary new location is not part of this recovery flow.

**Your app journey** is a compact strip above the workspace. Open it for a focused guidance panel; it does not push the preview down. The stages are Ideate, Connect Supabase, Create, Add assets, Preview & test, Build and Publish. In Ideate, write a short brief and choose **Save & continue**, plan with the Assistant, or choose **Build from brief** to save and stage an implementation request. Build from brief appends to an existing unsent draft instead of replacing it. Closing the panel keeps an unsaved brief during the session. New-app setup saves the brief and the backend choice with the project. A project brief, idea acknowledgment and optional-step choices are stored in the project's bounded `.mobile-builder.json` metadata, outside generic source writes. They survive a fresh browser and registration under another local profile. They are shared project data, not private account notes; keep secrets out of the brief. Existing browser-only choices can be imported explicitly. Old browser-only test flags are not imported because they have no source revision.

A user-reported test completion records the current source revision. Later source changes mark those checks **Check again**; Studio never presents them as automated or physical-device evidence. Concurrent progress changes are revision-checked. An unsaved brief survives polling, and conflicting saved progress requires reviewing the retained draft against the latest values before saving. The guide's Build action opens the existing reviewed native preparation, and Publish opens Launch Kit. Those stages stay incomplete: signing, remote builds, installation verification, deployment and store submission still require their separate delivery work.
...
OpenAI image generation is optional. Configure it in **Settings → Image generation** for this Dunara session or supply the optional startup `OPENAI_API_KEY`. Assistant providers are selected separately in **AI connections**. Every billable image request needs explicit Studio approval of the displayed prompt, references, model and output settings, even if Claude staged it. Execution trust does not authorize spending; no automatic paid retries occur. Cancellation does not guarantee provider cancellation/refund. Imports, transformations and icons remain available without a key.
...
## Settings: AI connections and image generation

Choose Assistant providers in **Settings → AI connections** or the chat footer; each connection uses its own key or subscription. See the [Assistant guide](assistant.md). The following steps apply to OpenAI image generation and its optional OpenAI Assistant fallback.

1. Open **Settings → Image generation**, even with an empty project registry. Enter the key in the masked **OpenAI API key** field. Save for this session, or explicitly check **Remember on this computer** to restore it after restart. Never paste a secret into chat, project source or an agent prompt.
...
4. A session key replaces the startup environment provider. **Disconnect OpenAI** here disables image generation and the OpenAI Assistant fallback for this process even when an environment key exists. Dedicated OpenAI Assistant keys, ChatGPT/Grok sign-ins and other providers are unchanged. **Use startup environment key** restores that provider explicitly when available. Session-only replacement or disconnect removes any saved value. Restart forgets session changes and restores saved/startup credentials.
...
A completed Build turn offers **Try your app** when the preview is running, or **Start preview** when execution is available. The action opens Focus without restarting an already-running preview, retaining its interactive state. **Review screens** opens the existing overview; **Refine the design** stages a review-and-improve request and is disabled while another unsent draft is present. These actions describe current preview state, not an automatic quality certification. Plan, failed and interrupted turns retain their existing Build/Continue recovery actions.

The screen overview refreshes missing or source-stale captures once per revision while visible. It pauses capture while the Assistant is working in that app. Failures keep previous images and expose manual retry; Stop refresh remains respected. Captures use fresh browser contexts and do not reproduce the live iframe’s transient form or completion state.

Build turns receive the same detailed design and visual-review guidance as MCP builds. This asks for a distinctive direction, coherent domain content, working state, original code-native art or approved assets, and inspection at both phone sizes. It does not bypass generation spending approvals or turn offline fixtures into evidence of live model output. See [the Bonsai experience audit](bonsai-experience-gap-plan.md).
