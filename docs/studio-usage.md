...
2. Select a registered project or choose **+ New app**. Enter a name and an optional brief, then continue to **Backend**. Choose **Supabase** to save an account connection before creation, or explicitly choose **No backend for now**. Creating a Supabase-backed workspace opens Backend for selecting and reviewing its project connection; the wizard does not provision or approve remote changes. Names accept letters, numbers, spaces, periods, apostrophes and hyphens. Slugs are lowercase letters/numbers separated by single hyphens. Creation uses the wellness starter; cancel does not create a project. Duplicate slugs produce a retryable error.
...
**Your app journey** is a compact strip above the workspace. Open it for a focused guidance panel; it does not push the preview down. The stages are Ideate, Connect Supabase, Create, Add assets, Preview & test, Build and Publish. In Ideate, write a short brief and choose **Save & continue**, or plan with the Assistant. Closing the panel keeps an unsaved brief during the session. New-app setup saves the brief and the backend choice with the project. A project brief, idea acknowledgment and optional-step choices are stored in the project's bounded `.mobile-builder.json` metadata, outside generic source writes. They survive a fresh browser and registration under another local profile. They are shared project data, not private account notes; keep secrets out of the brief. Existing browser-only choices can be imported explicitly. Old browser-only test flags are not imported because they have no source revision.
