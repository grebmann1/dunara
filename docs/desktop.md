...
The same loopback Host/Origin checks, one-use bootstrap tickets, authenticated API/WebSocket transport and CSP used by browser Studio apply here. Browser storage uses a persistent desktop partition for Studio layout and generated-app data. Studio and each local preview retain their loopback ports across restarts; a busy saved port is reported without moving the app to another origin. Studio bearer tokens remain in renderer memory and are replaced on restart. Earlier ephemeral desktop partitions are not automatically migrated. Desktop launches with an allowlisted environment and blocks Node/Electron injection variables. `OPENAI_API_KEY`, `BUILDER_ASSISTANT_API_KEY`, `BUILDER_BACKEND_ENCRYPTION_KEY`, `BUILDER_ACCOUNT_SUPABASE_URL`, `BUILDER_ACCOUNT_PUBLISHABLE_KEY` and `BUILDER_ACCOUNT_ALLOW_LOCAL` are explicitly captured, validated and transferred privately to the backend; the Dunara `.env` (or `--builder-env-file`) is also supported. Assistant providers are configured independently under **Settings → AI connections**. Startup model keys supply only OpenAI image generation and its compatibility fallback for the OpenAI Assistant connection. **Settings → Image generation** defaults to session-only entry, with optional **Remember on this computer** using encrypted owner-only Dunara-home files backed by OS protection or an explicit configured encryption key. Locked storage retains existing data; unavailable protection permits session-only use. Saved keys override startup keys and are restored on restart; session-only replacements are forgotten. All service settings are excluded from managed app environments; only the selected app binding supplies its public Supabase URL/key/environment. Supabase remembered connections use encrypted storage and are retained unchanged if the original encryption key is unavailable. An absolute `NODE_EXTRA_CA_CERTS` path is forwarded to Node/npm; Node options and TLS verification overrides remain excluded. See [backend setup](backend-setup.md). See `docs/credentials.md` for setup, source precedence, deletion and same-user risks. Sending an Assistant message authorizes the selected provider for that turn; image generation requires separate Assets approval. Saving an API key makes no provider request. Subscription sign-in and refresh contact that provider.
