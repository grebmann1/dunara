...
test('assistant and design use side tabs, retain drafts and stay usable in compact windows', async ({ page }, info) => {
  await page.getByRole('button', { name: 'Assistant', exact: true }).click();
  const input = page.getByRole('textbox', { name: 'Message assistant' });
  await expect(input).toBeEditable(); await input.fill('A draft that stays with this panel');
  ...
  await dragPanel(page, 'Assistant', 'left');
  await expect(page.getByLabel('Corner radius')).toHaveValue('29');
  const canvas = (await page.locator('.workspace-content').boundingBox())!;
  const left = (await page.locator('[data-panel=assistant]').boundingBox())!;
  const right = (await page.locator('[data-panel=design]').boundingBox())!;
  expect(left.x + left.width).toBeLessThanOrEqual(canvas.x + 1); expect(canvas.x + canvas.width).toBeLessThanOrEqual(right.x + 1);
  expect(canvas.width).toBeGreaterThanOrEqual(418);
  await page.screenshot({ path: info.outputPath('assistant-and-design.png') });
  await page.getByLabel('Arrange Assistant panel', { exact: true }).click(); await page.keyboard.press('Escape');
  await expect(input).toBeVisible(); await expect(page.getByLabel('Arrange Assistant panel', { exact: true })).toBeFocused();
  ...
  await expect(page.getByLabel('Corner radius')).toHaveValue('29');
  await expect(input).toHaveValue('A draft that stays with this panel');
  await expect(page.locator('[data-panel=assistant]')).toHaveAttribute('data-position', 'left');
});

test('placement has keyboard alternatives and cancellation, and compact panels retain their drafts', async ({ page }, info) => {
  await page.getByRole('button', { name: 'Assistant', exact: true }).click();
  const grip = page.getByRole('button', { name: 'Drag Assistant panel' });
